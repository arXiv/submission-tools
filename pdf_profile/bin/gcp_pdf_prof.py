...
def list_outcome_yymms(yymms: typing.List[str], bucket: str):
    postfix = '.outcome.tar.gz'
    result = []
    for yymm in yymms:
        prefix = f'gs://{bucket}/ps_cache/arxiv/pdf/{yymm}/'
        ls_outcome = subprocess.Popen(['gsutil', 'ls', prefix + '*' + postfix],
                                      encoding="utf-8",
...
def list_blob_versions(blob_path: str):
    ls_outcome = subprocess.Popen(['gsutil', 'ls', "-al", blob_path],
                                  encoding="utf-8",
                                  stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    out, err = ls_outcome.communicate()
    result = []
...
def download_pdf_variants(xid, bucket, profile) -> None:
    ...
    for xid in xids:
        ...
        for pdf_path, vers in paths:
            ...
            subprocess.run(['gsutil', 'cp', pdf_path, dest], encoding="utf-8")
            if profile:
